...
public class LiteralExpression extends BaseTerminalExpression {
    ...
    static {
    	for (Determinism determinism : Determinism.values()) {
                NULL_EXPRESSIONS[determinism.ordinal()] = new LiteralExpression(null, null, ByteUtil.EMPTY_BYTE_ARRAY,
                        null, null, SortOrder.getDefault(), determinism);
	        for (int i = 0; i < PDataType.values().length; i++) {
	            PDataType type = PDataType.values()[i];
                TYPED_NULL_EXPRESSIONS[i+PDataType.values().length*determinism.ordinal()] = new LiteralExpression(null,
                        type, ByteUtil.EMPTY_BYTE_ARRAY, !type.isFixedWidth() ? null : type.getMaxLength(null), null,
                        SortOrder.getDefault(), determinism);
	        }
                BOOLEAN_EXPRESSIONS[determinism.ordinal()] = new LiteralExpression(Boolean.FALSE, PBoolean.INSTANCE,
                        PBoolean.INSTANCE.toBytes(Boolean.FALSE), !PBoolean.INSTANCE.isFixedWidth() ? null : PBoolean.INSTANCE.getMaxLength(null),
                        null, SortOrder.getDefault(), determinism);

                BOOLEAN_EXPRESSIONS[Determinism.values().length + determinism.ordinal()] = new LiteralExpression(Boolean.TRUE, PBoolean.INSTANCE,
                        PBoolean.INSTANCE.toBytes(Boolean.TRUE), !PBoolean.INSTANCE.isFixedWidth() ? null : PBoolean.INSTANCE.getMaxLength(null),
                        null, SortOrder.getDefault(), determinism);
    	}
    }
    ...
    public static class Builder {
        ...
        public LiteralExpression buildValueAndDeterminism(boolean byteValueOnly) {
            /***
             * @param byteValueOnly: true if building expression that only has byteValue, false otherwise
             */
            //for when you only have value (could be null) and determinism (can also be null)
            // all other fields should be null
            if (byteValueOnly) {
                return new LiteralExpression(null, null, this.byteValue = this.byteValue != null ? this.byteValue : ByteUtil.EMPTY_BYTE_ARRAY, null, null, SortOrder.ASC, Determinism.ALWAYS);
            }
            setDefaults();
            if (this.value instanceof Boolean) {
            return LiteralExpression.getBooleanLiteralExpression((Boolean)this.value, this.determinism);
            }
            ...
            if (type.isNull(this.byteValue)) {
                return LiteralExpression.getTypedNullLiteralExpression(type, this.determinism);
            }
            ...
            return new LiteralExpression(this.value, type, this.byteValue, type == null || !type.isFixedWidth() ? null :
                    type.getMaxLength(this.value), null, SortOrder.getDefault(), this.determinism);
        }

        public LiteralExpression build() throws SQLException{
            setDefaults();
            if (this.value == null) {
                return  (this.type == null) ?  LiteralExpression.getNullLiteralExpression(this.determinism) :
            ...
            try {
                ...
                throw TypeMismatchException.newException(this.type, actualType, this.value.toString());
            }
            this.byteValue = this.type.isArrayType() ? ((PArrayDataType)this.type).toBytes(this.value, PArrayDataType.arrayBaseType(this.type), this.sortOrder, this.rowKeyOrderOptimizable) :
                    this.type.toBytes(this.value, this.sortOrder);
            if (this.type == PVarchar.INSTANCE || this.type == PChar.INSTANCE) {
                if (this.type == PChar.INSTANCE && this.maxLength != null  && this.byteValue.length < this.maxLength) {
                    if (this.rowKeyOrderOptimizable) {
                        this.byteValue = this.type.pad(this.byteValue, this.maxLength, this.sortOrder);
                    } else {
            ...
            if (this.maxLength == null) {
                this.maxLength = this.type.isFixedWidth() ? this.type.getMaxLength(this.value) : null;
            }
            return new LiteralExpression(this.value, this.type, this.byteValue, this.maxLength, this.scale, this.sortOrder, this.determinism);
        }
    ...
    public boolean isBooleanNull(Expression child) {
        if (child!=null) {
    ...
    // tests were failing without this default constructor
    }
